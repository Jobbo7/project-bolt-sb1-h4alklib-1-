...
import { 
  ...
  Building, PlusCircle, ArrowUpRight, Play, BookOpen, Cpu, QrCode
} from 'lucide-react';
...
export default function App() {
  ...
  // SUPPLIER / SELLER STATE ENGINE
  const [wmsLinked, setWmsLinked] = useState(true);
  const [sellerOffers, setSellerOffers] = useState([
    { id: 'SKU-882-ROT', part: 'DBA Slotted Brake Rotors (Pair)', price: 210.00, stock: 14, location: 'Aisle 3-B' },
    { id: 'SKU-109-FIL', part: 'Ryco Oil Filter Suite Pack', price: 110.00, stock: 42, location: 'Aisle 1-A' }
  ]);

  // ON-SHELF MANAGEMENT STATE
  const [newItemName, setNewItemName] = useState('');
  const [newItemQty, setNewItemQty] = useState('');
  const [newItemLoc, setNewItemLoc] = useState('');

  // TRANSACTION & ROUTING INTEGRATION
  ...
  const handleCourierScan = (jobId) => {
    alert(`⚡ RAPID HOT-SHOT VALIDATION ACTIVE!\nScanning courier dispatch manifest QR token for job: ${jobId}`);
    setActiveJobCards(prev => prev.map(job => job.id === jobId ? { ...job, status: 'Delivery Verified (Hot-Shot Express)' } : job));
  };

  const handleGlobalQrHandshake = () => {
    alert(`📸 INITIALISING SCANNER...\nCryptographic handshake completed with courier terminal sequence.\nStatus: ST_DELIV_OK -> Package marked on-site.`);
  };
  ...
  const handleOrderExecution = (jobId, itemDesc, rawPrice) => {
    if (user?.role === 'APPRENTICE') {
      ...
      alert(`🚨 EMPLOYEE ROUTER ACTIVE!\nOrder held for authorization. Detailed manifest routed straight to supervisor: ${user.linkedAccount}`);
    } else {
      const directReceipt = {
        timestamp: new Date().toLocaleTimeString(),
        desc: itemDesc,
        price: rawPrice,
        jobId,
        operator: user?.role === 'DIY' ? 'DIY Retail Checkout' : 'Master Owner'
      };
      setCompletedTransactions(prev => [directReceipt, ...prev]);
      alert(`⚡ HOT-SHOT COURIER DISPATCHED!\nWMS matched trade SKU instantly. Order routed via priority courier directly to your bay.`);
    }
  };
  ...
  const handleOwnerApproveOrder = (reqId, action) => {
    ...
    if (action === 'APPROVE') {
      ...
      alert(`🟢 REQUISITION SIGNED OFF!\nStripe Live payment released. Automated WMS signal broadcasted to wholesale merchant shelf for rapid Hot-Shot delivery.`);
    } else {
      alert(`❌ Requisition dropped.`);
    }
