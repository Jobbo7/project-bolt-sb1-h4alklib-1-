        ...
        <div className="p-4 max-w-7xl mx-auto">
          ...
          <div className="rounded-xl border p-4 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-lg shadow-black/40" style={{ borderColor: C.border, background: C.panel }}>
            <div>
              ...
              {user.role === 'APPRENTICE' && <p className="text-[10px] text-slate-400 mt-0.5 font-mono">🔗 Supervisor Routing Destination: {user.linkedAccount}</p>}
            </div>
          ...
          {user.role === 'SELLER' ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <div className="rounded-xl border p-5 shadow-lg" style={{ borderColor: C.border, background: C.panel }}>
                  ...
                  <div className="flex flex-col gap-2.5">
                    {sellerOffers.map(offer => (
                      <div key={offer.id} className="p-3.5 rounded-xl border flex justify-between items-center bg-slate-950/40 border-slate-800/80" style={{ borderColor: C.border }}>
                        <div>
                          <div className="text-xs font-bold text-slate-200">{offer.part}</div>
                          <div className="text-[10px] text-slate-400 font-mono mt-0.5">{offer.id} | Available Inventory: {offer.stock} units</div>
                        </div>
              ...
              <div className="rounded-xl border p-5 shadow-lg" style={{ borderColor: C.border, background: C.panel }}>
                ...
                <div className="p-4 rounded-xl border text-center mb-4 bg-slate-950/40 border-slate-800" style={{ borderColor: C.border }}>
                  <div className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Settled Payout Vault</div>
                  <div className="text-2xl font-bold font-mono text-emerald-400 mt-1">$4,120.00</div>
                  <div className="text-[9px] text-slate-500 uppercase tracking-widest mt-1 font-mono">Live via Stripe Connect</div>
                </div>
              </div>
            </div>
                ...
                <div className="rounded-xl border p-4 shadow-md" style={{ borderColor: C.border, background: C.panel }}>
                  ...
                  <div className="flex flex-col gap-2 max-h-48 overflow-y-auto pr-1">
                    {inventoryList.map(stock => (
                      <div key={stock.id} className="p-2.5 rounded-lg border flex justify-between items-center text-xs bg-slate-950/40 border-slate-800/80" style={{ borderColor: C.border }}>
                        <div className="truncate pr-2">
                          <div className="font-semibold text-slate-300 truncate">{stock.item}</div>
                          <div className="text-[9px] text-slate-400 font-mono mt-0.5">{stock.id} | Loc: <span className="text-slate-200 font-semibold">{stock.location}</span></div>
                        </div>
                    ...
                    {filteredJobs.map(job => (
                      <div key={job.id} className="p-4 rounded-xl border flex flex-col justify-between shadow-sm transition-all border-slate-800/80 bg-slate-900/40 hover:border-slate-700/60">
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1 leading-relaxed"><span className="text-slate-400 font-medium">Active Assignment:</span> {job.currentTask}</p>
                        </div>
...
function AuthGate({ onAuthenticate, isAuthenticating }) {
  ...
  const handleAuthSubmission = (e) => {
    ...
    if (isSignUpMode) {
      ...
      alert(`🟢 REGISTRATION COMPLETE!\nAccount saved to local memory database cache.`);
      setIsSignUpMode(false);
      setPassword('');
    } else {
      const savedAccountRaw = localStorage.getItem(storageKey);
      if (!savedAccountRaw) {
        setAuthError('❌ ACCESS DENIED: Account profile not found on this node. Please toggle to Sign Up mode first.');
        return;
      }

      const verifiedProfile = JSON.parse(savedAccountRaw);
            if (verifiedProfile.password !== password.trim()) {
        setAuthError('❌ SECURITY EXCEPTION: Invalid secure password credentials.');
        return;
      }
