...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log(
      `📡 PartsForge parts search: "${cleanQuery}" for ` +
    ...
    try {
      ...
      if (catalogueResponse.ok) {
        ...
        console.log(
          `📚 Catalogue result: ` +
          `${catalogueParts.length} authoritative part(s)`
        );
      } else {
        console.warn(
          `⚠️ Catalogue endpoint returned HTTP ` +
    ...
    return res.status(200).json({
      ...
      catalogue: {
        providerReady:
          catalogueData?.providerReady ===
          true,

        source:
          catalogueData?.source ||
          null,
        ...
        authoritativeMatches:
          catalogueParts.length,

        message:
          catalogueData?.message ||
          null
      },
    ...
    return res.status(500).json({
      error:
        'Parts search infrastructure failure.',
